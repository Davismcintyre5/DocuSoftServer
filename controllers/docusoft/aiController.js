...
exports.getContext = async (req, res) => {
  try {
    ...
    const context = {
      ...
      documents: documents.map(d => ({ title: d.title, price: d.price, isFree: d.isFree, description: d.description?.substring(0, 200) })),
      software: software.map(s => ({ title: s.title, price: s.price, isFree: s.isFree, description: s.description?.substring(0, 200) })),
      pricing: { freeItems: [...documents, ...software].filter(i => i.isFree).length, paidItems: [...documents, ...software].filter(i => !i.isFree).length, currency: settings.currency || 'KES' },
...
exports.chat = async (req, res) => {
  try {
    ...
    // Fetch settings WITH apiKey (select: false bypass)
