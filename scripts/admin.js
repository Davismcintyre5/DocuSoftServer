...
const listAdmins = async () => {
  ...
  if (currentApp.name === 'HDM Portfolio') {
    User = connection.model('HdmUser', new mongoose.Schema({}, { strict: false }));
    ...
    User = connection.model('User', new mongoose.Schema({}, { strict: false }));
...
const createAdmin = async () => {
  ...
  if (currentApp.name === 'HDM Portfolio') {
    const User = connection.model('HdmUser', new mongoose.Schema({}, { strict: false }));
...
const main = async () => {
  while (true) {
    ...
    Object.entries(APPS).forEach(([key, app]) => {
      console.log(`  ${app.color}${key}. ${app.name}\x1b[0m`);
    });
    console.log('  0. Exit');
